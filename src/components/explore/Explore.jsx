            ...
            getItems.map((item, index) => (
              <div key={index} className="explore-card">
                <div className='featured-card'>
                        ...
                        <div className='pfp-info-wrap'>
                            <div>
                                ...
                                <div>{name}</div>
                                {
                                  royaltyReceiver ?
                                  <div>
                                    <small>Royalty Receiver: {royaltyReceiver.slice(0,2)+"..."+royaltyReceiver.slice(38,43)}</small>
                                  </div>
                                  :
                                  <></>
                                }
                    ...
                    <div>
                      <button className='explore-buy-btn' onClick={(e)=>buyMarketItem(item)}>
                        Buy For {ethers.utils.formatEther(Number(item.totalPrice).toString())} ETH
                      </button>
                    </div>
                </div>
              </div>
